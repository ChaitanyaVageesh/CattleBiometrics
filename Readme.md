...
An (MLOps) pipeline designed to identify individual cattle using their unique muzzle prints. Muzzle ridges and beads function similarly to human fingerprints, providing a tamper-proof biometric signature.
...
Traditional ML Baseline: Support Vector Machine (SVM) using Histogram of Oriented Gradients (HOG) feature extraction.

Shallow Neural Network: Multi-Layer Perceptron (MLP).

Deep Transfer Learning (Final): DenseNet169 / ResNet50. Unfreezing the top layers for fine-tuning allowed the model to map specific cattle ridges, while label_smoothing=0.1 was utilized in the loss function to act as a powerful regularizer against overconfidence.
